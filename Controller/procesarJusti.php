    ...
    if (isset($_FILES['evidencia']) && $_FILES['evidencia']['error'] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['evidencia']['tmp_name'], $rutaArchivo)) {
            echo "El archivo se ha subido exitosamente.";
             // Instanciar modelos
             $modeloEvidencia = new Evidencia($conexion);
             $modeloJustificante = new Justificante($conexion);
             $modeloJustificanteProfesor = new JustificanteProfesor($conexion); 
             $modeloProfesor = new Profesor($conexion); 
             ...
             $idEvi = $conexion->insert_id; // Obtener el ID de la evidencia insertada
